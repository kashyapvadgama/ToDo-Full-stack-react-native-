...
// Component ke props define karein
  ...
  return (
    <View style={styles.taskItem}>
        ...
        <View style={styles.taskTextContainer}>
            ...
            {item.description ? <Text style={[styles.taskDescription, item.completed && styles.completedText]}>{item.description}</Text> : null}
        ...
        <TouchableOpacity onPress={() => onEdit(item)} style={{marginRight: 15}}><Icon name="edit-2" size={20} color="#888" /></TouchableOpacity>
        <TouchableOpacity onPress={() => onDelete(item._id)}><Icon name="trash-2" size={20} color="#E57373" /></TouchableOpacity>
    </View>
  );
};

// Styles ko bhi yahan le aayein
